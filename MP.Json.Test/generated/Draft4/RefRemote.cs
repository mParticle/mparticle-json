...
namespace JsonSchemaTestSuite.Draft4
{
    public class RefRemote
    {
        ...
        public void BaseURIChangeChangeFolder(string desc, string data, bool expected)
        {
            ...
            Console.Error.WriteLine(desc);
            string schemaData = "{ 'definitions':{ 'baz':{ 'id':'folder/', 'items':{ '$ref':'folderInteger.json' }, 'type':'array' } }, 'id':'http://localhost:1234/scope_change_defs1.json', 'properties':{ 'list':{ '$ref':'#/definitions/baz' } }, 'type':'object' }";
            MPJson schemaJson = MPJson.Parse(schemaData);
            MPJson json = MPJson.Parse(data);
            MPSchema schema = schemaJson;
            var validator = new JsonValidator { Strict = true, Version = SchemaVersion.Draft4 };
        ...
        public void BaseURIChangeChangeFolderInSubschema(string desc, string data, bool expected)
        {
            ...
            Console.Error.WriteLine(desc);
            string schemaData = "{ 'definitions':{ 'baz':{ 'definitions':{ 'bar':{ 'items':{ '$ref':'folderInteger.json' }, 'type':'array' } }, 'id':'folder/' } }, 'id':'http://localhost:1234/scope_change_defs2.json', 'properties':{ 'list':{ '$ref':'#/definitions/baz/definitions/bar' } }, 'type':'object' }";
            MPJson schemaJson = MPJson.Parse(schemaData);
            MPJson json = MPJson.Parse(data);
            MPSchema schema = schemaJson;
            var validator = new JsonValidator { Strict = true, Version = SchemaVersion.Draft4 };
