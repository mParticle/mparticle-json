...
namespace JsonSchemaTestSuite.Draft4
{
    public class Minimum
    {
        ...
        /// <summary>
        ///     2 - minimum validation (explicit false exclusivity)
        /// </summary>
        ...
        public void MinimumValidationExplicitFalseExclusivity(string desc, string data, bool expected)
        {
            // minimum validation (explicit false exclusivity)
            Console.Error.WriteLine(desc);
            string schemaData = "{ 'exclusiveMinimum':false, 'minimum':1.1 }";
            MPJson schemaJson = MPJson.Parse(schemaData);
            MPJson json = MPJson.Parse(data);
            MPSchema schema = schemaJson;
            var validator = new JsonValidator { Strict = true, Version = SchemaVersion.Draft4 };
        ...
        public void ExclusiveMinimumValidation(string desc, string data, bool expected)
        {
            ...
            Console.Error.WriteLine(desc);
            string schemaData = "{ 'exclusiveMinimum':true, 'minimum':1.1 }";
            MPJson schemaJson = MPJson.Parse(schemaData);
            MPJson json = MPJson.Parse(data);
            MPSchema schema = schemaJson;
            var validator = new JsonValidator { Strict = true, Version = SchemaVersion.Draft4 };
        ...
        /// <summary>
        ///     4 - minimum validation with signed integer
        /// </summary>
