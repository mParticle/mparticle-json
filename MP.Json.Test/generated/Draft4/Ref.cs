...
namespace JsonSchemaTestSuite.Draft4
{
    public class Ref
    {
        ...
        public void EscapedPointerRef(string desc, string data, bool expected)
        {
            // escaped pointer ref
            Console.Error.WriteLine(desc);
            string schemaData = "{ 'percent%field':{ 'type':'integer' }, 'properties':{ 'percent':{ '$ref':'#/percent%25field' }, 'slash':{ '$ref':'#/slash~1field' }, 'tilde':{ '$ref':'#/tilde~0field' } }, 'slash/field':{ 'type':'integer' }, 'tilde~field':{ 'type':'integer' } }";
            MPJson schemaJson = MPJson.Parse(schemaData);
            MPJson json = MPJson.Parse(data);
            MPSchema schema = schemaJson;
            var validator = new JsonValidator { Strict = true, Version = SchemaVersion.Draft4 };
        ...
        /// <summary>
        ///     9 - Recursive references between schemas
        /// </summary>
        ...
        public void RecursiveReferencesBetweenSchemas(string desc, string data, bool expected)
        {
            ...
            Console.Error.WriteLine(desc);
            string schemaData = "{ 'definitions':{ 'node':{ 'description':'node', 'id':'http://localhost:1234/node', 'properties':{ 'subtree':{ '$ref':'tree' }, 'value':{ 'type':'number' } }, 'required':[ 'value' ], 'type':'object' } }, 'description':'tree of nodes', 'id':'http://localhost:1234/tree', 'properties':{ 'meta':{ 'type':'string' }, 'nodes':{ 'items':{ '$ref':'node' }, 'type':'array' } }, 'required':[ 'meta', 'nodes' ], 'type':'object' }";
            MPJson schemaJson = MPJson.Parse(schemaData);
            MPJson json = MPJson.Parse(data);
            MPSchema schema = schemaJson;
            var validator = new JsonValidator { Strict = true, Version = SchemaVersion.Draft4 };
        ...
        /// <summary>
        ///     11 - Location-independent identifier
        /// </summary>
        ...
        /// <summary>
        ///     12 - Location-independent identifier with absolute URI
        /// </summary>
        ...
        /// <summary>
        ///     13 - Location-independent identifier with base URI change in subschema
        /// </summary>
        ...
        public void LocationIndependentIdentifierWithBaseURIChangeInSubschema(string desc, string data, bool expected)
        {
            ...
            Console.Error.WriteLine(desc);
            string schemaData = "{ 'allOf':[ { '$ref':'http://localhost:1234/nested.json#foo' } ], 'definitions':{ 'A':{ 'definitions':{ 'B':{ 'id':'#foo', 'type':'integer' } }, 'id':'nested.json' } }, 'id':'http://localhost:1234/root' }";
            MPJson schemaJson = MPJson.Parse(schemaData);
            MPJson json = MPJson.Parse(data);
            MPSchema schema = schemaJson;
            var validator = new JsonValidator { Strict = true, Version = SchemaVersion.Draft4 };
