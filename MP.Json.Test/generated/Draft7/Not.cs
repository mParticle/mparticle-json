...
namespace JsonSchemaTestSuite.Draft7
{
    public class Not
    {
        ...
        /// <summary>
        ///     5 - not with boolean schema true
        /// </summary>
        ...
        public void NotWithBooleanSchemaTrue(string desc, string data, bool expected)
        {
            // not with boolean schema true
            Console.Error.WriteLine(desc);
            string schemaData = "{ 'not':true }";
            MPJson schemaJson = MPJson.Parse(schemaData);
            MPJson json = MPJson.Parse(data);
            MPSchema schema = schemaJson;
            var validator = new JsonValidator { Strict = true, Version = SchemaVersion.Draft7 };
        ...
        /// <summary>
        ///     6 - not with boolean schema false
        /// </summary>
        ...
        public void NotWithBooleanSchemaFalse(string desc, string data, bool expected)
        {
            // not with boolean schema false
            Console.Error.WriteLine(desc);
            string schemaData = "{ 'not':false }";
            MPJson schemaJson = MPJson.Parse(schemaData);
            MPJson json = MPJson.Parse(data);
            MPSchema schema = schemaJson;
            var validator = new JsonValidator { Strict = true, Version = SchemaVersion.Draft7 };
