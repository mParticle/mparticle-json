...
namespace JsonSchemaTestSuite.Draft201909
{
    public class RefRemote
    {
        ...
        public void BaseURIChangeChangeFolder(string desc, string data, bool expected)
        {
            ...
            Console.Error.WriteLine(desc);
            string schemaData = "{ '$defs':{ 'baz':{ '$id':'folder/', 'items':{ '$ref':'folderInteger.json' }, 'type':'array' } }, '$id':'http://localhost:1234/scope_change_defs1.json', 'properties':{ 'list':{ '$ref':'#/$defs/baz' } }, 'type':'object' }";
            MPJson schemaJson = MPJson.Parse(schemaData);
            MPJson json = MPJson.Parse(data);
            MPSchema schema = schemaJson;
            var validator = new JsonValidator { Strict = true, Version = SchemaVersion.Draft201909 };
        ...
        public void BaseURIChangeChangeFolderInSubschema(string desc, string data, bool expected)
        {
            ...
            Console.Error.WriteLine(desc);
            string schemaData = "{ '$defs':{ 'baz':{ '$defs':{ 'bar':{ 'items':{ '$ref':'folderInteger.json' }, 'type':'array' } }, '$id':'folder/' } }, '$id':'http://localhost:1234/scope_change_defs2.json', 'properties':{ 'list':{ '$ref':'#/$defs/baz/$defs/bar' } }, 'type':'object' }";
            MPJson schemaJson = MPJson.Parse(schemaData);
            MPJson json = MPJson.Parse(data);
            MPSchema schema = schemaJson;
            var validator = new JsonValidator { Strict = true, Version = SchemaVersion.Draft201909 };
        ...
        public void RootRefInRemoteRef(string desc, string data, bool expected)
        {
            ...
            Console.Error.WriteLine(desc);
            string schemaData = "{ '$id':'http://localhost:1234/object', 'properties':{ 'name':{ '$ref':'name-defs.json#/$defs/orNull' } }, 'type':'object' }";
            MPJson schemaJson = MPJson.Parse(schemaData);
            MPJson json = MPJson.Parse(data);
            MPSchema schema = schemaJson;
            var validator = new JsonValidator { Strict = true, Version = SchemaVersion.Draft201909 };
