...
namespace JsonSchemaTestSuite.Draft201909
{
    public class Items
    {
        ...
        public void ItemsAndSubitems(string desc, string data, bool expected)
        {
            // items and subitems
            Console.Error.WriteLine(desc);
            string schemaData = "{ '$defs':{ 'item':{ 'additionalItems':false, 'items':[ { '$ref':'#/$defs/sub-item' }, { '$ref':'#/$defs/sub-item' } ], 'type':'array' }, 'sub-item':{ 'required':[ 'foo' ], 'type':'object' } }, 'additionalItems':false, 'items':[ { '$ref':'#/$defs/item' }, { '$ref':'#/$defs/item' }, { '$ref':'#/$defs/item' } ], 'type':'array' }";
            MPJson schemaJson = MPJson.Parse(schemaData);
            MPJson json = MPJson.Parse(data);
            MPSchema schema = schemaJson;
            var validator = new JsonValidator { Strict = true, Version = SchemaVersion.Draft201909 };
