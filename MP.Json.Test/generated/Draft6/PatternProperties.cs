...
namespace JsonSchemaTestSuite.Draft6
{
    public class PatternProperties
    {
        ...
        /// <summary>
        ///     4 - patternProperties with boolean schemas
        /// </summary>
        ...
        public void PatternPropertiesWithBooleanSchemas(string desc, string data, bool expected)
        {
            // patternProperties with boolean schemas
            Console.Error.WriteLine(desc);
            string schemaData = "{ 'patternProperties':{ 'b.*':false, 'f.*':true } }";
            MPJson schemaJson = MPJson.Parse(schemaData);
            MPJson json = MPJson.Parse(data);
            MPSchema schema = schemaJson;
            var validator = new JsonValidator { Strict = true, Version = SchemaVersion.Draft6 };
